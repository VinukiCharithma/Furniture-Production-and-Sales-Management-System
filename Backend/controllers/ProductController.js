...
const addProduct = async (req, res, next) => {
    ...
    try {
        product = new Product({ name, category, price, material, availability, image });
        await product.save();
    } catch (err) {
        console.log(err);
        return res.status(500).json({ message: "Server error, unable to add product" });
    }
...
const updateProduct = async (req, res, next) => {
    ...
    try {
        ...
        console.log(err);
        return res.status(500).json({ message: "Server error, unable to update product" });
    }
...
const deleteProduct = async (req, res, next) => {
    ...
    try {
        ...
        console.log(err);
        return res.status(500).json({ message: "Server error, unable to delete product" });
    }
