...
const AdminProduct = () => {
    ...
    const fetchViewCounts = async (productList) => {
        for (let product of productList) {
            try {
                const response = await axios.get(`http://localhost:5000/api/product/viewcount/${product._id}`);
                setViewCounts(prev => ({
                    ...prev,
    ...
    const toggleAvailability = async (productId, newAvailability) => {
        try {
            await axios.put(`http://localhost:5000/products/${productId}`, { availability: newAvailability });
